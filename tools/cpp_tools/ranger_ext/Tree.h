...
class Tree {
 public:
  ...
  void init(const Data* data, uint mtry, size_t num_samples, uint seed,
            ...
            bool holdout, uint num_random_splits, uint max_depth,
  ...
  enum class PrintType {
    FEATURE_COUNT = 0,
    COMPLETE = 1,
    SINGLE_TREE_MAXDEPTH = 2,
    DEPTH = 3
  };

  void print_complete();

  void print_complete(size_t node_id);

  void print_feature_count();

  void print_feature_count(size_t node_id,
                           std::unordered_map<size_t, size_t>& map);

  void print_single_tree_maxdepth(size_t node_id, size_t maxdepth);

  void print_depth();

  void print_depth(size_t node_id, size_t curr_depth);
 ...
 protected:
  ...
  virtual bool splitNodeInternal(
      size_t nodeID, std::vector<size_t>& possible_split_varIDs) = 0;
  ...
  // Minimum node size to split, like in original RF nodes of smaller size can
  // be produced
  uint min_node_size;
