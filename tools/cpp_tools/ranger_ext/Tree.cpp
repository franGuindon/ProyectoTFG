...
Tree::Tree()
    : mtry(0),
      ...
      num_samples_oob(0),
      min_node_size(0),
      deterministic_varIDs(0),
      split_select_weights(0),
      case_weights(0),
      manual_inbag(0),
      oob_sampleIDs(0),
      holdout(false),
...
#define PRINT_FEATURE_USE

void Tree::print_feature_count() {
  std::unordered_map<size_t, size_t> tree_map;

  print_feature_count(0, tree_map);
...
void Tree::print_feature_count(size_t node_id,
  ...
  print_feature_count(child_nodeIDs[0][nodeID], map);
  print_feature_count(child_nodeIDs[1][nodeID], map);
}

void Tree::print_complete() { print_complete(0); }

void Tree::print_complete(size_t node_id) {
  size_t nodeID = node_id;
  ...
  print_complete(child_nodeIDs[0][nodeID]);
  printf("Node %ld: Moving to right child node\n", nodeID);
  print_complete(child_nodeIDs[1][nodeID]);
}

void Tree::print_single_tree_maxdepth(size_t node_id, size_t maxdepth) {
  size_t nodeID = node_id;
  ...
  printf("Node %ld: Moving to left child node\n", nodeID);
  print_single_tree_maxdepth(child_nodeIDs[0][nodeID], maxdepth - 1);
  printf("Node %ld: Moving to right child node\n", nodeID);
  print_single_tree_maxdepth(child_nodeIDs[1][nodeID], maxdepth - 1);
}

void Tree::print_depth() { print_depth(0, 0); }

void Tree::print_depth(size_t node_id, size_t curr_depth) {
  size_t nodeID = node_id;

  if (child_nodeIDs[0][nodeID] == 0 && child_nodeIDs[1][nodeID] == 0) {
    printf("Leaf depth: %ld\n", curr_depth);
    return;
  }

  print_depth(child_nodeIDs[0][nodeID], curr_depth + 1);
  print_depth(child_nodeIDs[1][nodeID], curr_depth + 1);
}
